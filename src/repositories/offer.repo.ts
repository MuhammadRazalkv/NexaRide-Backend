...
export class OfferRepo extends BaseRepository<IOffer> implements IOfferRepo {
  ...
  async findOfferUsage(userId: string, offerId: string): Promise<IOfferUsage | null> {
    return await OfferLimit.findOne({ userId, offerId });
  }
  async increaseOfferUsage(userId: string, offerId: string): Promise<IOfferUsage | null> {
