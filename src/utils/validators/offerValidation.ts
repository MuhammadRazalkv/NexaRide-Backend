...
export default function validateOfferData(data: IOffer): string | null {
  ...
  if (data.type === 'percentage') {
    if (data.value > 90 || data.value < 0) return 'Percentage value should be between 1 - 90';
    if (!data.maxDiscount || data.maxDiscount <= 0)
      return 'Max discount is required for percentage offers and must be > 0.';
  }

  if (data.minFare !== undefined && data.minFare < 0) return 'Minimum fare cannot be negative.';
  ...
  if (data.validTill <= data.validFrom) return 'Valid till must be after valid from.';
