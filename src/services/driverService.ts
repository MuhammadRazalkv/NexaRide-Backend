...
const driverSchema = z.object({
    ...
    license_number: z.string()
        ...
        .regex(/^[A-Z]{2}\d{2} \d{4} \d{7}$/, "License number must contain only uppercase letters and digits"),
...
class DriverService {
    ...
    async emailVerification(email: string) {
        ...
        OTPRepo.sendOTP(email, "Your NexaRide OTP", html(OTP),).catch((error) => { throw new Error(error) })
    }
    ...
    async addInfo(data: IDrivers) {
        ...
        const newDriver = await driverRepo.createDriver(parsedData.data)
    ...
    async login(driverData: IDrivers) {
        try {
            ...
            // const vehicle_id = driver.vehicle_id ? driver.vehicle_id.toString() : ''
            // const vehicle = await vehicleRepo.findVehicleById(vehicle_id)
    ...
    async checkGoogleAuth(id: string, email: string) {
        ...
        if (existingDriver) {
            ...
            // If Google ID matches, allow login
            const message = 'User already exists, logging in...'
            return message
