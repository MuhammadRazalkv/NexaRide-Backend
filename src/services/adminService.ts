...
import { generateAdminAccessToken, generateAdminRefreshToken, generateRefreshToken } from "../utils/jwt";
import sendEmail from "../utils/mailSender";
import { driverApprovalEmail, vehicleApprovalEmail, rejectionEmail } from "../constants/OTP";
...
class AdminService {
    ...
    async toggleBlockUnblockDriver(id: string) {
        try {
            ...
            const updatedDriver = await driverRepo.blockUnblockDriver(id, driver.isBlocked);

            if (updatedDriver) {
                return {
                    message: updatedDriver.isBlocked ? "Driver blocked successfully" : "Driver unblocked successfully",
    ...
    async rejectDriver(id: string, reason: string) {
        try {
            ...
            if (updatedDriver) {
                await sendEmail(updatedDriver.email, 'Driver rejection', rejectionEmail(updatedDriver.name, updatedDriver?.rejectionReason || 'Due to incorrect info', 'Driver'))
                return {
    ...
    async rejectVehicle(id: string, reason: string) {
        try {
            ...
            if (updatedVehicle) {
                await sendEmail(driver.email, 'Vehicle rejection', rejectionEmail(driver.name, updatedVehicle.rejectionReason || 'Due to incorrect info', 'Vehicle'))
                return {
